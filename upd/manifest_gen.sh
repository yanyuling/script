...
resVer="`svn info | grep Revision|awk   '{printf $2}'`"
echo "资源版本$resVer"
allmd5=""

fileList="$(find ../../src -name "*.*")" || exit 1
for i in $fileList; do
    echo ${i#*../../}
    if [[ -f $DIR/$i && ! $i =~ "DS_Store" ]]; then
        fileMd5="$(md5 -q $DIR/$i )" || exit 1
        allmd5="$allmd5, \"${i#*../../}\":{\"md5\":\"$fileMd5\"}" || exit 1
    fi
done
fileList="$(find ../../res -name "*.*")" || exit 1
for i in $fileList; do
    echo ${i#*../../}
    if [[ -f $DIR/$i && ! $i =~ "DS_Store" ]]; then
        fileMd5="$(md5 -q $DIR/$i )" || exit 1
...
# echo "{\"packageUrl\" : \"$gateUrl/clientweb/patch/debug/number/\",\"remoteManifestUrl\" : \"$gateUrl/clientweb/patch/debug/number/project.manifest\",\"remoteVersionUrl\" : \"$gateUrl/clientweb/patch/debug/version.manifest\",\"version\" : \"$resVer\",\"engineVersion\" : \"3.x dev\"}" > version.manifest || exit 1